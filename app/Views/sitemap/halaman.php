	...
	<div id="content">
		...
		<xsl:if test="count(sitemap:sitemapindex/sitemap:sitemap) &gt; 0">
			...
			<table id="sitemap" cellpadding="3">
				...
				<tbody>
					<tr>
						<td><a href="<?= base_url('simulasi-kpr'); ?>"><loc><?= base_url('simulasi-kpr'); ?></loc></a></td>
						<td><lastmod><?= date('Y-m-d') ?></lastmod></td>
					</tr>
					<?php foreach($data as $m): ?>
					<tr>
						<td><a href="<?= base_url('halaman/'.$m->url_page); ?>"><loc><?= base_url('halaman/'.$m->url_page); ?></loc></a></td>
						<td><lastmod><?= date('Y-m-d') ?></lastmod></td>
					</tr>
					<?php endforeach; ?>
					<tr>
						<td><a href="<?= base_url(); ?>"><loc><?= base_url(); ?></loc></a></td>
						<td><lastmod><?= date('Y-m-d') ?></lastmod></td>
					</tr>
				</tbody>
			</table>
